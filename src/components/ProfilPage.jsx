...
export default function RechargeVirement() {
  ...
  const dettagliBancari = {
    beneficiario: user?.nome || "CARTA NERA SAS",
    iban: user?.iban || "IT60 X054 2811 1010 0000 0123 456",
    bic: user?.bic || "BPPIITRRXXX",
    banca: user?.agenzia || "BNL - Banca Nazionale del Lavoro",
    riferimento: user?.id ? `RICARICA-${user.id}` : "RICARICA-XXXXXX",
    codiceFiscale: user?.codiceFiscale || "N/D",
    numeroCarta: user?.numeroCarta || "N/D",
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "16px 16px 40px" }}>
        ...
        {/* Dati personali per il bonifico */}
        <div style={{ background: PP.white, borderRadius: 16, padding: "16px 18px", marginBottom: 16, boxShadow: "0 2px 8px rgba(0,48,135,0.07)" }}>
          <p style={{ color: PP.blue, fontWeight: 700, fontSize: 14, margin: "0 0 14px" }}>Dati personali</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <InfoRow etichetta="Codice Fiscale" valore={dettagliBancari.codiceFiscale} campo="codiceFiscale" campoCopito={campoCopito} onCopy={handleCopy} />
            <InfoRow etichetta="Numero Carta" valore={dettagliBancari.numeroCarta} campo="numeroCarta" campoCopito={campoCopito} onCopy={handleCopy} />
          </div>
        </div>
